...
const BottomNav = memo(({ task, activeTab, onTaskChange, onTabChange, onCaptureClick, onUploadClick, fileInputRef, handleImageUpload }: BottomNavProps) => {
    return (
        <div className="safe-bottom flex flex-col items-center bg-gradient-to-t from-black/95 via-black/70 to-transparent pb-6 pt-10 space-y-6">

            {/* Capture Button */}
            <button
                onClick={onCaptureClick}
                className="group relative w-20 h-20 rounded-full flex items-center justify-center transition-all active:scale-90 touch-manipulation shadow-[0_0_30px_rgba(255,255,255,0.25)]"
            ...
            >
                ...
                <div className="w-16 h-16 bg-white rounded-full border-[5px] border-slate-300 shadow-inner group-hover:scale-105 transition-transform"></div>
            </button>
            ...
            <input type="file" ref={fileInputRef} onChange={handleImageUpload} className="hidden" accept="image/*" aria-label="Choose image file" />

            {/* Bottom Navigation Bar */}
            <div className="flex items-center justify-around w-full px-6 max-w-sm mx-auto" role="navigation" aria-label="Modes">
                ...
                <button
                    ...
                    aria-label="Describe Scene Mode"
                >
